...
void rijndaelEncrypt(const u32 *rk, int nrounds, u8 plaintext[16],
		u8 ciphertext[16])
{
...
AES_ROUND_KEYS_128_128 aes_roundkey_128;
AES_ROUND_KEYS_128_256 aes_roundkey_256;


inline void aes_encrypt_b128_k256_one_block( u8 plaintext[16], const u32 *rk )
{
	rijndaelEncrypt( rk, 14, plaintext, plaintext);
}

#define aes_encrypt_b128_k128_one_block( plaintext, rk )  { \
		u32 s0, s1, s2, s3, t0, t1, t2, t3;  \
		s0 = GETU32(plaintext     ) ^ rk[0]; \
		s1 = GETU32(plaintext +  4) ^ rk[1]; \
		s2 = GETU32(plaintext +  8) ^ rk[2]; \
...
//void aes_encrypt_b128_k128_one_block( u8 plaintext[16], const u32 *rk );

void Test_AES_128_128_Speed_Expanded_Memory() {

	u32 *rk = (u32 *) aes_roundkey_128;
	unsigned long long tricky_compiler = 0;

	// round keys are already set by test vectors call
	//	rijndaelSetupEncrypt(rk, (unsigned char *) aes_k_exp_128, 128);

	for (unsigned int jj = 0; jj < number_of_reencryptions_in_memory; jj++)
		for (unsigned int ii = 0; ii < number_of_blocks_in_memory_128; ii++) {
			aes_encrypt_b128_k128_one_block( ( u8 * )&encrypted_memory.block128[ ii ], rk );
...
{
	u32 *rk = (u32 *) aes_roundkey_256;

	// round keys are already set by test vectors call
	//	rijndaelSetupEncrypt(rk, (unsigned char *) aes_k_exp_256, 256);

	for (unsigned int jj = 0; jj < number_of_reencryptions_in_memory; jj++)
		for (unsigned int ii = 0; ii < number_of_blocks_in_memory_128; ii++)
			aes_encrypt_b128_k256_one_block( (u8 *) &encrypted_memory.block128[ii], rk );
}

void aes128_test_vectors()
{
	unsigned char aestestkey[] = { 0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
			0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f };
	unsigned char aestest[] = { 0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,
			0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff };

	u32 *rk = (u32 *) aes_roundkey_128;

	rijndaelSetupEncrypt(rk, aestestkey, 128);
	...
	aes_encrypt_b128_k128_one_block( aestest, rk );
...
void aes256_test_vectors()
{
	...
	rijndaelSetupEncrypt(rk, aestestkey, 256);
	...
	aes_encrypt_b128_k256_one_block( aestest, rk );
